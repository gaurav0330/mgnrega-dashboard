...
const STATE_NAME = process.env.STATE_NAME || 'MAHARASHTRA';
...
// Schedule monthly data ingestion on 1st of each month at 2 AM
cron.schedule('0 2 1 * *', async () => {
  console.log('Running scheduled monthly data ingestion...');
  ...
  try {
    await ingestDataForMonth(STATE_NAME, year, month);
    console.log(`✅ Scheduled ingestion completed for ${month}/${year}`);
  } catch (err) {
    console.error('❌ Scheduled ingestion failed:', err);
  }
});

console.log('Scheduler started. Monthly ingestion will run on 1st of each month at 2 AM');
